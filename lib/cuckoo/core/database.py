...
try:
    ...
    from sqlalchemy import Integer, String, Boolean, DateTime, Enum, or_, and_
...
class Task(Base):
    ...
    def __repr__(self):
        print "below is self fucking id"
        print self.id
...
class Database(object):
    ...
    def add(self, obj, timeout=0, package="", options="", priority=1,
            ...
            memory=False, enforce_timeout=False, clock=None, user_status=False, user_id=1):
        ...
        try:
            session.commit()
            task_id = task.id
            print "after commit task id is %s" % task_id
        except SQLAlchemyError as e:
            log.debug("Database error adding task: {0}".format(e))
            session.rollback()
            return None
    ...
    def add_path(self, file_path, timeout=0, package="", options="",
        ...
        return self.add(File(file_path), timeout, package, options, priority,
                        ...
                        enforce_timeout, clock, user_status, user_id)
    ...
    def add_url(self, url, timeout=0, package="", options="", priority=1,
                ...
                enforce_timeout=False, clock=None, user_status=False, user_id=1):
    ...
    @classlock
    def list_tasks(self, taskid=None, limit=None, details=False, category=None,
                   offset=None, status=None, sample_id=None, not_status=None,
                   completed_after=None, order_by=None, user_status=None, user_id=None, not_this_user=None):
        ...
        try:
            ...
            if taskid:
                search = search.filter_by(id=taskid)
            if status:
            ...
            if user_id:
                search = search.filter_by(user_id=user_id)
            if not_this_user:
                search = search.filter(Task.user_id!=not_this_user,Task.user_status==1)
            if user_status==0 or user_status==1:
                search = search.filter_by(user_status=user_status)
