...
def get_analysis_info(db, id=-1, task=None):
    ...
    if id>0:
        new["base64"] = until.encrpt(id)
    else:
        new["base64"] = until.encrpt(new["id"])
...
def report(request, task_id):
    ...
    print "decrpt_task_id: %s." % decrpt_task_id
    db = Database()

    if request.user.is_authenticated():
       checkprviate = db.list_tasks(taskid=decrpt_task_id, not_this_user=request.user.id)
       if len(checkprviate)>0:
           return render_to_response("error.html",
                                     {"error": "无权访问，此扫描结果为用户隐私!!!"},
                                     context_instance=RequestContext(request))
    if not request.user.is_authenticated():
       checkprviate = db.list_tasks(taskid=decrpt_task_id, user_status=0)
       if len(checkprviate)==0:
           return render_to_response("error.html",
                                     {"error": "无权访问，此扫描结果为用户隐私!!!"},
                                     context_instance=RequestContext(request))
...
def search(request):
    if "search" in request.POST:
        ...
        for result in records:
            ...
            if not new:
                continue
            if (new["user_id"]==request.user.id and new["user_status"]==1) or new["user_status"]==0:
                analyses.append(new)
        return render_to_response("analysis/search.html",
                                  {"analyses": analyses,
                                   "config": enabledconf,
